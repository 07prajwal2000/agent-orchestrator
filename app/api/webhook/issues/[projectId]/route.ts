...
import { issueWebhookSchema } from "@/app/lib/schemas";
	...
	try {
		...
		if (!isModifierAllowed) {
			return NextResponse.json(
				{
					message:
						"Ignoring event: editor/sender is not in the allowed modification list.",
				},
				{ status: 200 },
			);
		}
		...
		const systemPrompt = SystemPromptBuilder.fromIssuePayload(
			payload,
			projectId,
		);

		// Spawn AI Agent asynchronously using worker thread
		console.log("System Prompt Generated for Issue:", systemPrompt);
		...
		return NextResponse.json(
			{ message: "Issue webhook received and processed successfully." },
			{ status: 200 },
		);
	} catch (error) {
		console.error("Error processing issue webhook:", error);
		return NextResponse.json(
